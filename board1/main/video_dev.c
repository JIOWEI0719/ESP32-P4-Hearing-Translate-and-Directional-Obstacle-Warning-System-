...
static void log_v4l2_pix_format(const char *prefix, const struct v4l2_pix_format *pix)
{
    char fourcc[5] = {
        (char)(pix->pixelformat & 0xff),
        (char)((pix->pixelformat >> 8) & 0xff),
        (char)((pix->pixelformat >> 16) & 0xff),
        (char)((pix->pixelformat >> 24) & 0xff),
        0,
    };

    ESP_LOGI(TAG,
             "%s: %ux%u fmt=%s bytesperline=%u sizeimage=%u",
             prefix,
             (unsigned)pix->width,
             (unsigned)pix->height,
             fourcc,
             (unsigned)pix->bytesperline,
             (unsigned)pix->sizeimage);
}
...
esp_err_t video_start(int width, int height, camera_context *cb_ctx) {
    ...
    ESP_ERROR_CHECK(ioctl(cb_ctx->cap_fd, VIDIOC_G_FMT, &format));
    log_v4l2_pix_format("capture format", &format.fmt.pix);
    cb_ctx->width = format.fmt.pix.width;
    cb_ctx->height = format.fmt.pix.height;
    cb_ctx->capture_stride = format.fmt.pix.bytesperline ? format.fmt.pix.bytesperline : cb_ctx->width;
    cb_ctx->capture_sizeimage = format.fmt.pix.sizeimage;
    ...
    format.fmt.pix.width = cb_ctx->width;
    format.fmt.pix.height = cb_ctx->height;
    format.fmt.pix.pixelformat = capture_fmt;
    ESP_ERROR_CHECK(ioctl(cb_ctx->m2m_fd, VIDIOC_S_FMT, &format));
    ESP_ERROR_CHECK(ioctl(cb_ctx->m2m_fd, VIDIOC_G_FMT, &format));
    log_v4l2_pix_format("encoder input format", &format.fmt.pix);
    ...
    ESP_ERROR_CHECK(ioctl(cb_ctx->m2m_fd, VIDIOC_S_FMT, &format));
    ESP_ERROR_CHECK(ioctl(cb_ctx->m2m_fd, VIDIOC_G_FMT, &format));
    log_v4l2_pix_format("encoder output format", &format.fmt.pix);
...
frame_buffer_t *video_fb_get(camera_context *cb_ctx) {
    ...
    esp_err_t cache_ret = esp_cache_msync(cb_ctx->cap_buffer[cap_buf.index],
                                          cap_buf.bytesused,
    ...
    if (cb_ctx->raw_frame_cb) {
        cb_ctx->raw_frame_cb(cb_ctx->cap_buffer[cap_buf.index],
                             cap_buf.bytesused,
                             cb_ctx->width,
                             cb_ctx->height,
                             cb_ctx->capture_stride,
                             cb_ctx->raw_frame_user_ctx);
    }
